...
from .gaussian_diffusion_x0_enhancement_RR import GaussianDiffusion_RR
...
class SpacedDiffusion(GaussianDiffusion_RR):
    """
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        base_diffusion = GaussianDiffusion_RR(**kwargs)  # pylint: disable=missing-kwoa
...
class _WrappedModel1:
    ...
    def __call__(self, x, x_reflection, ts, light_factor_0, light_mask_0, light_factor_45, light_mask_45,light_factor_90, light_mask_90,corner, **kwargs):
        map_tensor = th.tensor(self.timestep_map, device=ts.device, dtype=ts.dtype)
        new_ts = map_tensor[ts]
        if self.rescale_timesteps:
            new_ts = new_ts.float() * (1000.0 / self.original_num_steps)
        return self.model(x, x_reflection, new_ts, light_factor_0, light_mask_0, light_factor_45, light_mask_45,light_factor_90, light_mask_90, corner, **kwargs)
